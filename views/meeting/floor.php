<div class="site-index my-5">
    <div class="container col-10">
        <div class="fs-3 text-center p-3 fw-bold"><?= Yii::t('app', 'Meeting Room Floor') ?></div>
        <div class="">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div>
                            ...
                            <select class="form-control rtypeSelection" required multiple id="roomTypeSelectionEdit" ></select>
                        </div>
                        <div>
...
<script>
    $(function()
    {
        ...
        var selectorView, selectorRoom, selectorType, rtypeSelection, allrtype, selectorFloorList;
    ...
    // number only
        ...
        $("#roomAssign").on("keypress", ":input[type='numeric']", function(e) 
        {
            var charCode = (e.which) ? e.which : event.keyCode    
            if (String.fromCharCode(charCode).match(/[^0-9]/g))  
            {
        ...
        $("#submitAddFloor").on("click",function()
        {
            ...
            else
            {
                ...
                $.ajax({
                    ...
                    data : data,
                    dataType : 'JSON',
                    url : '<?= Yii::$app->getUrlManager()->createUrl('meeting-floor/add-floor') ?>',
    ...
    // edit logic
            ...
            { 
                ...
                // set input
                    ...
                    for (let i = 0; i < totalQty; i++) 
                    {
                        $("#roomAssign").append("\
                                    <div class=\"py-2 border border-4 border-dark rounded mt-2 px-3\">\
                                        ...
                                        <div>\
                                            ...
                                            <input type=\"text\" class=\"form-control\" required id=\"roomName" + (i+1) + "\" placeholder=\"<?= Yii::t('app', 'Enter Room Name') ?>\">\
                                        ...
                                        <div>\
                                            <label for=\"roomCapacity" + (i+1) + "\" class=\"form-label\"><?= Yii::t('app', 'Room Capacity') ?></label>\
                                            <input type=\"numeric\" class=\"form-control\" required id=\"roomCapacity" + (i+1) + "\" placeholder=\"<?= Yii::t('app', 'Enter Room Capacity') ?>\">\
                                            <div class=\"invalid-feedback\"><?= Yii::t('app', 'Please input room capacity.') ?></div>\
                                            <div class=\"valid-feedback\"></div>\
                                        </div>\
                                        <div>\
                                            <label for=\"tableQty" + (i+1) + "\" class=\"form-label\"><?= Yii::t('app', 'Table Qty') ?></label>\
                                            <input type=\"numeric\" class=\"form-control\" required id=\"tableQty" + (i+1) + "\" placeholder=\"<?= Yii::t('app', 'Enter Table Qty') ?>\">\
                                            <div class=\"invalid-feedback\"><?= Yii::t('app', 'Please input table qty.') ?></div>\
                                            <div class=\"valid-feedback\"></div>\
                                        </div>\
                                        <div>\
                                            <label for=\"chairQty" + (i+1) + "\" class=\"form-label\"><?= Yii::t('app', 'Chair Qty') ?></label>\
                                            <input type=\"numeric\" class=\"form-control\" required id=\"chairQty" + (i+1) + "\" placeholder=\"<?= Yii::t('app', 'Enter Chair Qty') ?>\">\
                                            <div class=\"invalid-feedback\"><?= Yii::t('app', 'Please input chair qty.') ?></div>\
                                            <div class=\"valid-feedback\"></div>\
                                        </div>\
                                            <div class='imgContainerfloor mt-2 border border-2 justify-content-center' id='floorMap" + (i+1) + "'>\
                                            </div>\
                                            <div class='pt-3'>\
                                            <input type=\"file\" accept=\"image/*\" id=\"inputimg" + (i+1) + "\">\
                                            </div>\
                                    </div>")
                        ...
                        if (floorListRoom[selector] != null) 
                        {
                            rname = floorListRoom[selector].ROOM.split(',');
                            table = floorListRoom[selector].TABLE_QTY.split(',');
                            capacity = floorListRoom[selector].CAPACITY.split(',');
                            chair = floorListRoom[selector].CHAIR_QTY.split(',');
                            ...
                            if (floorListRoom[selector].ROOM_MAP != null) 
                            {
                                rmap = floorListRoom[selector].ROOM_MAP.split(',');
                                if (rmap[i] != undefined) 
                                {
                                    $("#floorMap"+(i+1)).html("<img class='imgRoomMapfloor' src='../../web/images/" +  rmap[i] + "'>")
                                }
                                else
                                {
                                    $("#floorMap"+(i+1)).html("No Image") 
                                }
                            }
                            else
                            {
                            ...
                            $("#roomCapacity"+(i+1)).val(capacity[i])
                            $("#tableQty"+(i+1)).val(table[i])
                            $("#chairQty"+(i+1)).val(chair[i])
                        }
                        else
                        {
                    ...
                    for (let i = 0; i < totalQty; i++) 
                    {
                        $("#roomAssign").append("\
                                    <div class=\"py-2 border border-4 border-dark rounded mt-2 px-3\">\
                                        ...
                                        <div class=\"pt-2\">\
                                        <input type=\"file\" accept=\"image/*\" id=\"roomMap" + (i+1) + "\" required>\
                                        </div>\
                                        </div>\
                                    </div>")
                        $("#roomType" + (i+1)).html(rtypeSelection);
                        ...
                        if (floorListAll[selector].ROOM != null) 
                        {
                            ...
                            table = floorListAll[selector].TABLE_QTY.split(',');
                            capacity = floorListAll[selector].CAPACITY.split(',');
                            chair = floorListAll[selector].CHAIR_QTY.split(',');
                            ...
                            $("#tableQty"+(i+1)).val(table[i])
                            $("#chairQty"+(i+1)).val(chair[i])
                            if (floorListAll[selector].ROOM_MAP != null) 
                            {
                                console.log(rmap)
                                rmap = floorListAll[selector].ROOM_MAP.split(',');
                                if (rmap[i] != undefined) 
                                {
                                    $("#floorMap"+(i+1)).html("<img class='imgRoomMapfloor' src='../../web/images/" +  rmap[i] + "'>")
                                }
        ...
        function EditFloor()
        {
            ...
            {
                availStatus = floorListRoom[selectorFloorList].ROOM_STATUS.split(',');
            }
            else 
            {
                flagger = 2;
        ...
        function ajaxEdit()
        {
            ...
            $.ajax({
                ...
                data : data,
                dataType : 'JSON',
                url : '<?= Yii::$app->getUrlManager()->createUrl('meeting-floor/edit-floor') ?>',
                async: false,
                success : function(result)
                {
                    if (result.errNum == 0) 
                    {
                        if (floorListAll[selectorFloorList].ROOM != null) 
                        {
                            ...
                            if (floorListAll[selectorFloorList].ROOM_MAP != null) 
                            {
                                mapArr = floorListAll[selectorFloorList].ROOM_MAP.split(",");
                            }
                            else
                            {
                                mapArr = []
                            ...
                            for (let i = 0; i < looplimit; i++) 
                            {
                                ...
                                if (i >= qtyMax && qtyMax < existinglength) 
                                {
                                    ...
                                    $.ajax({
                                        ...
                                        data : data,
                                        dataType : 'JSON',
                                        url : '<?= Yii::$app->getUrlManager()->createUrl('meeting-floor/remove-room') ?>',
                                        success : function(result)
                                        {
                                            if (result.errNum == 1)
                                            {
                                ...
                                {
                                    input = document.getElementById("inputimg"+(i+1))
                                    filename = "";
                                    if (input != null ) 
                                    {
                                        if (input.files[0] != undefined) 
                                        {
                                            fileinp = input.files[0];
                                            formData = new FormData();
                                            formData.append("image", fileinp);
                                            $.ajax({
                                                url: '<?= Yii::$app->getUrlManager()->createUrl('meeting-floor/image-upload') ?>',
                                                type: "POST",
                                                data: formData,
                                                processData: false,
                                                contentType: false,
                                            ...
                                            filename = fileinp.name;
                                        }
                                    }
                                    temp = $("#roomType"+(i+1)).val().split(',');
                                    if (temp[0] == "" || temp[1] == undefined || $("#roomName"+(i+1)).val() == "" || $("#roomCapacity"+(i+1)).val() == "" || $("#tableQty"+(i+1)).val() == "" || $("#chairQty"+(i+1)).val() == "") 
                                    {
                                        var data = {
                                            ...
                                            capacity : $("#roomCapacity"+(i+1)).val(),
                                            tableQty : $("#tableQty"+(i+1)).val(),
                                            chairQty : $("#chairQty"+(i+1)).val(),
                                            map : filename,
                                            status : 'unavailable',
                                            adminId : adminId,
                                        };
                                    ...
                                    $.ajax({
                                        ...
                                        data : data,
                                        dataType : 'JSON',
                                        url : '<?= Yii::$app->getUrlManager()->createUrl('meeting-floor/add-room') ?>',
                                        async: false,
                                        success : function(result)
                                        {
                                            if (result.errNum == 1)
                                ...
                                else
                                {
                                    ...
                                    $.ajax({
                                        ...
                                        data : data,
                                        dataType : 'JSON',
                                        url : '<?= Yii::$app->getUrlManager()->createUrl('meeting-floor/edit-room') ?>',
                                        async: false,
                                        success : function(result)
                                        {
                                            if (result.errNum == 1)
                        ...
                        else
                        {
                            for (let i = 0; i < qtyMax; i++) 
                            {
                                ...
                                temp = $("#roomType"+(i+1)).val().split(',');
                                var data = {
                                    floorId : floorListAll[selectorView].ID,
                                    rtypeId : temp[1],
                                    rtypeName : temp[0],
                                    name : $("#roomName"+(i+1)).val(),
                                    capacity : $("#roomCapacity"+(i+1)).val(),
        ...
        function DeleteFloor()
        {
            ...
            $.ajax({
                ...
                data : data,
                dataType : 'JSON',
                url : '<?= Yii::$app->getUrlManager()->createUrl('meeting-floor/remove-floor') ?>',
                async: false,
                success : function(result)
                {
                    if (result.errNum == 1)
