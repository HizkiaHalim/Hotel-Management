<div class="site-index my-5">
    <div class="container col-10">
        <div class="fs-3 text-center p-3 fw-bold"><?= Yii::t('app', 'Meeting Room Facility') ?></div>
        <div class="">
        ...
        for (let i = 0; i < facilityList.length; i++) 
        {
           $("#facilityList").append("\
                            <tr class=\"text-center\">\
                                ...
                                <td class=\"align-middle\"><button type='button' class='editFacility btn btn-primary' data-bs-toggle='modal' data-bs-target='#formEdit' value=" + i + ">View</button></td>\
                            <tr>")       
        }
            ...
            $("#submitAddFacility").on('click',function()
            {
                ...
                else
                {
                    ...
                    $.ajax({
                        ...
                        data : data,
                        dataType : 'JSON',
                        url : '<?= Yii::$app->getUrlManager()->createUrl('meeting-facility/add-facility') ?>',
        ...
        //Edit Logic
            ...
            $("#submitEdit").on('click', function()
            {
                ...
                $.ajax({
                    ...
                    data : data,
                    dataType : 'JSON',
                    url : '<?= Yii::$app->getUrlManager()->createUrl('meeting-facility/edit-facility') ?>',
                    success : function(result)
                    {
                        if (result.errNum == 1)
                        {
        ...
        //Delete
            $("#submitDelete").on('click', function()
            {
                ...
                $.ajax({
                    ...
                    data : data,
                    dataType : 'JSON',
                    url : '<?= Yii::$app->getUrlManager()->createUrl('meeting-facility/delete-facility') ?>',
                    success : function(result)
                    {
                        if (result.errNum == 0) 
                        {
