...
class HotelBooking extends Model
{
    ...
    public $bookingId = 0;
    public $name, $adminId, $id, $totalPrice, $transactionDate, $bookBy, $bookerPhone, $checkIn, $checkOut, $bookQty, $roomIdArr, $priceArr, $breakfastArr, $extrabedArr, $roomStatus, $paymentId;
...
//-----------------------------------------GET--------------------------------------
    ...
    public function getRtypeWithCount()
    {
        ...
        $sql = "
                ...
                FROM 
                    HIZ_HOTEL_RTYPE R 
                LEFT JOIN 
                    HIZ_HOTEL_ROOM H ON R.ID = H.RTYPE_ID 
                WHERE 
                    R.STATUS = 1 
                    AND H.STATUS = 1
    ...
    public function getRtypeAvbInFloor()
    {
        ...
        $sql = "
                ...
                LEFT JOIN 
                    HIZ_HOTEL_RTYPE R ON H.RTYPE_ID = R.ID
                LEFT JOIN
                    HIZ_HOTEL_PRICE P ON R.ID = P.RTYPE_ID 
                WHERE 
                    F.STATUS = 1 
                    AND H.STATUS = 1 
                    AND H.ROOM_STATUS = 'available'
    ...
    public function getTransactionFullDetailed()
    {
        ...
        $sql = "     
                SELECT 
                    ...
                    LISTAGG ( F.NAME, ',' ) WITHIN GROUP (ORDER BY F.FLOOR_LEVEL ) AS FLOOR_NAME,
    ...
    public function addBooking()
    {
        ...
        //booking detail + update status room
            for ($i=0; $i < sizeof($this->roomIdArr) ; $i++) 
            {
                ...
                $sql = "
                        BEGIN
                            sp_hiz_hotel_bookdetail_insert
                            (
                                out_code		            => :outCode,
                                out_msg			            => :outMsg,
                                in_book_id			        => :bookingId,
                                in_room_id                  => :roomId,
                                in_book_extrabed            => :extrabed,
                ...
                $st->bindParam(':extrabed', $this->extrabedArr[$i]);
                ...
                $sql = "
                        BEGIN
                            sp_hiz_hotel_room_updateBooked
                            (
                                out_code		            => :outCode,
                                out_msg			            => :outMsg,
                                in_id			            => :roomId,
