...
class MeetingFloor extends Model
{
    public $errNum = 0;
    public $errStr = '';
    public $name, $level, $qty, $adminId, $column, $row, $id, $floorId, $rtypeId, $rtypeName, $status, $rtypeIdArr, $rtypeNameArr, $nameArr, $capacity, $tableQty, $chairQty, $map, $capacityArr, $tableQtyArr, $chairQtyArr, $mapArr, $roomId;

    public function rules()
    {
        return[
            // rule room
            [['floorId', 'rtypeId', 'rtypeName', 'name', 'status', 'adminId', 'capacity', 'tableQty', 'chairQty'], 'required', 'on' => ['scenarioAddRoom']],
    ...
    public function getFloorRoom()
    {
        ...
        $sql = "
                SELECT 
                    ...
                    LISTAGG ( H.CAPACITY, ',' ) WITHIN GROUP (ORDER BY H.ROOM_NAME ) AS CAPACITY, 
                    LISTAGG ( H.TABLE_QTY, ',' ) WITHIN GROUP (ORDER BY H.ROOM_NAME ) AS TABLE_QTY, 
                    LISTAGG ( H.CHAIR_QTY, ',' ) WITHIN GROUP (ORDER BY H.ROOM_NAME ) AS CHAIR_QTY, 
    ...
    public function getFloorRtype()
    {
        ...
        $sql = "
                ...
                FROM 
                    HIZ_MEETING_FLOOR F
                LEFT JOIN
                    HIZ_MEETING_RTYPE R ON F.RTYPE_ID = R.ID
                WHERE 
...
//-----------------------------------------ADD ROOM--------------------------------------
    ...
    public function addRoom()
    {
        ...
        $sql = "
                BEGIN
                    ...
                    (
                        ...
                        in_table_qty                => :tableQty,
                        in_chair_qty                => :chairQty,
                        in_room_map                 => :map,
        ...
        $st->bindParam(':name', $this->name);
        $st->bindParam(':capacity', $this->capacity);
        $st->bindParam(':tableQty', $this->tableQty);
        $st->bindParam(':chairQty', $this->chairQty);
        $st->bindParam(':map', $this->map);
